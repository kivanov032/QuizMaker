...
import {
    sendQuestionsToFixError,
    sendQuestionsToRecordInBD,
    sendQuestionsToSearchError,
} from '../SenderQuizCreating.js';
...
export default function CreatorLayout() {
    ...
    const [expandedSections, setExpandedSections] = useState({
        quizErrors:
            quizErrors?.name_quiz_errors && Object.keys(quizErrors.name_quiz_errors).length > 0, // Раскрыть, если есть ошибки викторин
        questionErrors:
            quizErrors?.critical_errors?.length > 0 ||
            quizErrors?.cosmetic_errors?.length > 0 ||
            quizErrors?.logical_errors?.length > 0 ||
            quizErrors?.minor_errors?.length > 0, // Раскрыть, если есть ошибки вопросов
...
>>>>>>> dimaer198
    ...
    const processResponse = (response) => {
        ...
        setExpandedSections({
            quizErrors:
                hasCriticalQuizErrors ||
                (response?.name_quiz_errors && Object.keys(response.name_quiz_errors).length > 0),
            questionErrors:
                hasCriticalQuestionErrors ||
                response?.critical_errors?.length > 0 ||
                response?.cosmetic_errors?.length > 0 ||
                response?.logical_errors?.length > 0 ||
                response?.minor_errors?.length > 0,
            criticalQuizErrors: hasCriticalQuizErrors,
            syntaxQuizErrors: false,
            criticalQuestionErrors: hasCriticalQuestionErrors,
            logicQuestionErrors: false,
...
>>>>>>> dimaer198
            ...
            console.error('Ошибка при отправке вопросов:', error);
            alert(
                'Техническая ошибка: невозможно произвести анализ викторины на присутствие ошибок.',
            );
        }
    };
...
>>>>>>> dimaer198
    ...
    const handleConfirmFinishQuiz = async () => {
        try {
            ...
            const response = await sendQuestionsToRecordInBD(
                quizName,
                questions,
                checkboxes,
                user.login,
                ...
                {quizErrors.name_quiz_errors &&
                    Object.keys(quizErrors.name_quiz_errors).length > 0 && (
                        <div className="error-section">
                            <h3
                                onClick={() => toggleSection('quizErrors')}
                                ...
                                <div className="error-subsections">
                                    ...
                                    {quizErrors.name_quiz_errors.critical_error && (
                                        <div className="error-subsection">
                                            <h4
                                                onClick={() => toggleSection('criticalQuizErrors')}
                                    ...
                                    {quizErrors.name_quiz_errors.cosmetic_error && (
                                        <div className="error-subsection">
                                            <h4
                                                onClick={() => toggleSection('syntaxQuizErrors')}
                                                className="subsection-title"
                                            >
                                                Косметические{' '}
...
>>>>>>> dimaer198
                ...
                {(quizErrors.critical_errors?.length > 0 ||
                    ...
                    <div className="error-section">
                        <h3
                            onClick={() => toggleSection('questionErrors')}
                            className="section-title"
                        >
...
<<<<<<< HEAD
                                ...
                                {quizErrors.critical_errors &&
                                    quizErrors.critical_errors.length > 0 && (
                                        <div className="error-subsection">
                                            <h4
                                                onClick={() =>
                                                    toggleSection('criticalQuestionErrors')
                                                }
                                                className="subsection-title"
                                            >
                                                Критические{' '}
                                                {expandedSections.criticalQuestionErrors
                                                    ? '▼'
                                                    : '▶'}
                                            </h4>
                                            {expandedSections.criticalQuestionErrors &&
                                                quizErrors.critical_errors.map(
                                                    (questionError, index) =>
                                                        questionError.errors.map(
                                                            (error, errorIndex) => (
                                                                <div
                                                                ...
                                                                >
                                                                    <span style={{ color: 'red' }}>
                                                                        -{' '}
                                                                        <Link
                                                                            to={`/createQuestion/${questionError.id_question}`}
                                                                            style={{
                                                                                color: 'red',
                                                                                textDecoration:
                                                                                    'underline',
                                ...
                                {quizErrors.logical_errors &&
                                    quizErrors.logical_errors.length > 0 && (
                                        <div className="error-subsection">
                                            <h4
                                                onClick={() => toggleSection('logicQuestionErrors')}
                                                className="subsection-title"
                                            >
                                                Логические{' '}
                                                {expandedSections.logicQuestionErrors ? '▼' : '▶'}
                                            </h4>
                                            {expandedSections.logicQuestionErrors &&
                                                quizErrors.logical_errors.map(
                                                    (questionError, index) =>
                                                        questionError.errors.map(
                                                            (error, errorIndex) => (
                                                                <div
                                ...
                                {/* Косметические ошибки (свернуты по умолчанию)*/}
                                {quizErrors.cosmetic_errors &&
                                    quizErrors.cosmetic_errors.length > 0 && (
                                        <div className="error-subsection">
                                            <h4
                                                onClick={() =>
                                                    toggleSection('syntaxQuestionErrors')
                                                }
                                                className="subsection-title"
                                            >
                                                Косметические{' '}
                                                {expandedSections.syntaxQuestionErrors ? '▼' : '▶'}
                                            </h4>
                                            {expandedSections.syntaxQuestionErrors &&
                                                quizErrors.cosmetic_errors.map(
                                                    (questionError, index) =>
                                                        questionError.errors.map(
                                                            (error, errorIndex) => (
                                                                <div
...
>>>>>>> dimaer198
                                {quizErrors.minor_errors && quizErrors.minor_errors.length > 0 && (
                                    <div className="error-subsection">
                                        <h4
                                            onClick={() => toggleSection('minorQuestionErrors')}
                                            className="subsection-title"
                                        >
                                            Несущественные{' '}
                    ...
                    <div className="modal-content">
                        ...
                        {hasErrors_NOT_significant && !hasErrors_significant && (
                            <div>
                                ...
                                <p>
                                    В викторине нет критических ошибок, но есть ряд других ошибок:
                                </p>
                                ...
                                {quizErrors?.minor_errors && quizErrors.minor_errors.length > 0 && (
                                    <p>- несущественные ошибки (будут исправлены автоматически).</p>
                                )}
                                {((quizErrors?.cosmetic_errors &&
                                    quizErrors.cosmetic_errors.length > 0) ||
                                    (quizErrors?.logical_errors &&
                                    ...
                                    !quizErrors?.minor_errors?.length > 0 && (
                                        <p>
                                            Перед нажатием на Кнопку Подтвердить можете выбрать{' '}
                                            <br />
                                            автоматическое исправление выше представленных типов
                                            ошибок.
                                        </p>
                                    )}
                        ...
                        {hasErrors_significant && (
                            <div>
                                ...
                                <p>
                                    В викторине присутствуют критические ошибки. Завершение
                                    викторины невозможно.
                                </p>
                                <p>
                                    Исправьте критические ошибки, чтобы завершить данную викторину.
                                </p>
                                <button
                                    onClick={() => {
